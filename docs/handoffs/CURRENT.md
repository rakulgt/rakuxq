...
基线时间：`2026-09-26T14:14:00+08:00`
当前生产稳定版本：`v0.4.4-alpha.3`
当前源码开发版本：`v0.4.4-alpha.3`

## 最近完成

- 增加免费单层加速入口 `xq-fast.rakubank.com`：使用 Cloudflare 橙云，为手机 VPN 或境外出口上传提供独立路线，同时保留 `xq.rakubank.com` 国内直连入口及统一鉴权、限流、审计和匿名计时。
- v0.4.4-alpha.3 已发布：新旧健康检查均返回 `0.4.4a3`，新入口经 Cloudflare 返回正确入口头；142 KB 真实 JPEG 的源站接收耗时 72 ms、应用处理 330 ms、Nginx 总耗时 404 ms，公网客户端端到端约 2.0 秒。源站证书覆盖三个过渡期主机名，Certbot 自动续期 dry-run 通过。完整记录见 `docs/deployments/20260926-141400-v0.4.4-alpha.3.md`。
- 修正 v0.4.4-alpha.1 将棋盘方向反转误当作阵营反转的问题：帅、将身份现在不可变；黑方在下时旋转整盘坐标，不再把帅改成将、将改成帅。
- 新增帅/将颜色锚点聚类：两枚主帅颜色可可靠区分时，使用图内颜色距离保守复核其他已知棋子阵营；证据不足时不修改，并完整保留原始类别与颜色距离依据。
- v0.4.4-alpha.2 已发布到正式域名：以用户刚上传的真实失败原图在服务器内部重跑，系统判定 `black_bottom` 并旋转坐标，帅、将身份完整保留，五项位置违法归零且颜色锚点未产生过度纠正；健康检查为 `0.4.4a2`，服务 `active`、`NRestarts=0`。完整记录见 `docs/deployments/20260926-112452-v0.4.4-alpha.2.md`。
- v0.4.4-alpha.1 曾尝试以全盘阵营交换处理该失败样本，但真实复测证明它会改变帅、将身份；该策略已由 v0.4.4-alpha.2 的坐标旋转与颜色锚点方案完全取代，历史部署事实保留在 `docs/deployments/20260925-213601-v0.4.4-alpha.1.md`。
...
- 增加 `vpn.xq.rakubank.com` 免费路线对照入口的源站配置：与正式域名共享 API，响应披露入口主机名，匿名时序日志可按主机比较；正式 `xq.rakubank.com` 继续保持灰云直连。
