...
基线时间：`2026-09-18T20:12:00+08:00`
当前生产稳定版本：`v0.2.1-alpha.1`
当前源码开发版本：`v0.3.0-alpha.1`
...
1. 用 20–50 个合法局面建立引擎回归集，覆盖普通评分、红黑行棋、将死、无合法着法、非法 FEN、超时和进程恢复。
2. 决定生产 NNUE 权重路线：取得 Pikafish 远程服务授权，或训练/恢复权利清晰的 RakuXQ 自有权重；在此之前不发布生产引擎服务。
3. 增加 ICCS 到规范中文着法的转换、多个候选变化和缓存，再更新 Apple 快捷指令的第二个“推荐下一步”入口。
4. 继续建立首批 20–50 张授权视觉测试集，评测整盘零错误率、自动通过覆盖率和自动通过精确率。
