...
基线时间：`2026-09-25T21:40:02+08:00`
当前生产稳定版本：`v0.4.4-alpha.1`
当前源码开发版本：`v0.4.4-alpha.1`

## 最近完成

- 新增可审计的整盘阵营反转纠正：只有唯一红帅、黑将交叉落入对方九宫且交换阵营后阻断性违法严格减少时才触发；原始网格、触发原因和纠正前后警告均保留，其他低置信问题仍要求复核。
- 将 2026-09-25 生产环境出现的金色同底棋子皮肤红黑反转 FEN 固化为回归用例；纠正后帅、将、士、象的阵营位置约束全部恢复合法。
- v0.4.4-alpha.1 已发布到正式域名：真实失败原图在服务器内部重跑后，红黑阵营与双方主帅九宫位置均已纠正，五项阵营位置违法归零；低置信棋种仍保持 `review_required`。健康检查为 `0.4.4a1`，服务 `active`、`NRestarts=0`。完整记录见 `docs/deployments/20260925-213601-v0.4.4-alpha.1.md`。
