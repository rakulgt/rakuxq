...
基线时间：`2026-09-17T18:32:15+08:00`
当前稳定版本：`v0.1.2-alpha.3`
...
- `python -m pytest -q`：40 项 FEN、局面校验、服务门控、鉴权与 Key 生命周期、HTTP 契约、短期审计、URL 行棋方别名、快速路径、部分棋盘和同图原型复核测试通过；另有 2 条来自测试客户端依赖的弃用提示，不影响运行时接口。
- `ruff check .`：通过。
- `mypy src`：13 个源文件通过。
...
- 生产运行状态：服务器发布标识与 GitHub 提交均为 `5344423b469198603c2a9d8f6b899cba79ac6d6a`；服务 `active`、`NRestarts=0`，短期清理 timer 为 `active`，Nginx 配置检查通过。
