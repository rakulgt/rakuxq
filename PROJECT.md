...
project_id: rakuxq
baseline_revision: "19"
parent_revision: "18"
change_operation: UPDATE
status: active
updated_at: "2026-09-26T14:46:50+08:00"
---

# RakuXQ

- 类型：`xiangqi-vision-engine-api`
...
- 第二阶段已启动：通过可替换 UCI 边界接入 NNUE 引擎，提供最佳着法、变化线和可复现搜索元数据；中文着法、解释及终端推送继续迭代。
...
- 首页示例棋盘是 RakuXQ 原生交互式局面研究器：由 FEN 初始化，允许查看合法落点、走子、吃子、悔棋、重置、复制当前 FEN 和跳转外部研究；不得通过裁剪第三方 iframe 维持核心交互。
...
- `帅(K)`和`将(k)`的字形身份不可由后处理互换。若唯一帅位于上方九宫、唯一将位于下方九宫，且旋转坐标可严格减少阻断性违法，自动将输入判为黑方在下并旋转整盘坐标；缺失主帅、单方越界或旋转后未改善时不得触发。
- 帅、将棋子区域可以建立本图红黑颜色锚点，但在独立完成字迹分割与多棋子一致性验证前，整块图像颜色只允许形成审计诊断，不得改写任何棋子的阵营大小写；尤其不得覆盖高置信原始分类。未来任何自动改色必须使局面合法性严格改善，否则立即撤销。
...
- Lab 桌面布局以常见 `1280×720` 视口完整看见整张棋盘为最低门槛；高频操作集中在常驻工具栏，品牌介绍不得挤占主要研究空间。
- “新局”固定表示结束当前研究并回到标准三十二子开局；回到研究根局面、分支首尾导航与自由摆子是不同操作，不得用含糊的单一“重置”按钮混在一起。
...
- 已确认：新增 `/v1/analyses` 处理已确认 FEN，新增 `/v1/solve` 串联自动通过的视觉结果；原 `/v1/recognitions` 和 `/v1/fen` 保持兼容。
