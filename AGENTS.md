...
- 自托管默认只在内存中处理图片。官方托管服务按 `PROJECT.md` 的短期审计规则保存已鉴权调用的原图和交互记录，最长 12 小时；不得进入 Git、长期样本集或自动训练集。
- `models/` 中的第三方权重必须有来源、版本、许可证和校验和记录；无法追溯的权重不得发布。
- 不得以规则修正掩盖视觉识别错误；原始候选、置信度和修正原因必须可审计。
- 官网匿名面板不得暴露原图、Key/Key ID、IP、文件名、请求 ID、设备信息或客户关联标识；逐条公开记录最多 72 小时，永久层只允许保存汇总数字。
...
- `/opt/raku/logs/rakuxq/public-metrics/` 是跨版本持久数据，不得随发布清空或放入 Git；快捷指令公开链接不得包含 API Key。
...
- 上述授权不包含强制推送、覆盖远端历史、改变仓库可见性、改远端所有者、删除 Release/仓库、发布第三方 ONNX 权重、把用户图片上传到第三方或永久保留用户图片，以及传播任何秘密；这些动作仍需单独确认。官方托管服务器内、最长 12 小时的短期审计留存已由用户明确授权。
